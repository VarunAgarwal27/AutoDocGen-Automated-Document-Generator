...
export default function Result() {
  ...
  const downloadFile = async (format) => {
    try {
      ...
      // 1. Check if the server response was successful
      if (!res.ok) {
        // Read the error message returned from the backend (if any)
        const errorData = await res.text();
        console.error("Backend error output:", errorData);
        showToast(`Download failed: Server returned status ${res.status}`, "error");
        return;
      }

      // 2. Only convert to blob if status is 2xx OK
      ...
      // 3. Revoke the object URL to clean up memory
